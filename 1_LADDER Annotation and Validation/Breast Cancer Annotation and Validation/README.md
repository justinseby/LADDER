# Breast Cancer Gene Set Pathway Annotation & Literature Validation Pipeline

This describes the three-stage pipeline used to annotate Breast Cancer gene sets with biological process labels and then validate those annotations against high-quality
published literature.

> **Scope:** This README covers the **Breast Cancer** pipeline only.
> follow the same overall design (annotation → knowledgebase → literature validation)

```
Breast Cancer gene communities (input)
        │
        ▼
┌───────────────────────────┐
│  Stage 1: Annotation      │
...
│                             │    single Breast Cancer literature database
└───────────────────────────┘
        │  Breast Cancer literature database
        ▼
┌───────────────────────────┐
│  Stage 3: Validation       │
...
For every Breast Cancer gene community, the notebook:
...
2. For each unique gene, queries PubMed (via NCBI E-utilities) restricted to Breast Cancer-specific MeSH terms
   and free-text disease terms , within a configurable publication-year window, retrieving
...
5. Consolidates every gene's papers into a **single unified Breast Cancer literature database** , tagging each paper row with the gene(s)/communities it was retrieved for, and
...
This stage produces the **Breast Cancer literature database** consumed by Stage 3
...
1. Loads the Breast Cancer literature database (Stage 2's output) and a journal-quality reference list, and tags
