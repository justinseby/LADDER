# Lung Cancer Gene Set Pathway Annotation & Literature Validation Pipeline

This describes the three-stage pipeline used to annotate Lung Cancer gene sets with biological process labels and then validate those annotations against high-quality
published literature.

> **Scope:** This README covers the **Lung Cancer** pipeline only.
> follow the same overall design (annotation → knowledgebase → literature validation)

```
Lung Cancer gene communities (input)
        │
        ▼
┌───────────────────────────┐
│  Stage 1: Annotation      │
...
│                             │    single Lung Cancer literature database
└───────────────────────────┘
        │  Lung Cancer literature database
        ▼
┌───────────────────────────┐
│  Stage 3: Validation       │
...
For every Lung Cancer gene community, the notebook:
...
2. For each unique gene, queries PubMed (via NCBI E-utilities) restricted to Lung Cancer-specific MeSH terms
...
5. Consolidates every gene's papers into a **single unified Lung Cancer literature database** , tagging each paper row with the gene(s)/communities it was retrieved for, and
...
This stage produces the **Lung Cancer literature database** consumed by Stage 3
...
1. Loads the Lung Cancer literature database (Stage 2's output) and a journal-quality reference list, and tags
